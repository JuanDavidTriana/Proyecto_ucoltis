...
from rest_framework.test import force_authenticate
from .models import BillingAddress, OrderModel, Model
...
class AccountApisSetUp(APITestCase):
    ...
    def setUp(self):
        ...
        # card of testuser
        self.testuser_card = Model.objects.create(
            email = "testuser@gmail.com",
        ...
        #  card of admin
        self.admin_card = Model.objects.create(
            email = "admin@gmail.com",
            name_on_card = "admin",
...
class AccountApisAuthTest(AccountApisSetUp):
    ...
    def test_fetching_of_user_card_when_logged_in(self):
        factory = APIRequestFactory()
        user = User.objects.get(username='testuser')
        view = CardsListView.as_view()

        request = factory.get('/account/cards/')
        force_authenticate(request, user=user)
        response = view(request)
        self.assertEqual(response.status_code, 200)
    ...
    def test_fetching_of_user_card_when_logged_out(self):
